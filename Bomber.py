...
def is_inside_building(building_num, pos, building_width, building_heights, building_xpos):
    if (pos[1] >= CANVAS_HEIGHT
        or pos[0] < building_xpos[building_num]
...
def shrink_building(canvas, building_num, building_width, building_heights, building_xpos, building_rects):
    ...
    if building_heights[building_num] > 0:
        ...
        building_heights[building_num] = 0
...
def init_display(root, plane_pos, bomb_pos, building_heights, building_xpos, building_rects):
    global building_width, game_running, canvas, won, msg_text_on
...
def create_buildings(canvas, building_width, building_heights, building_xpos, building_rects):
    ...
    if len(building_rects) > 0:
        for building_num in range(0, len(building_rects)):
            delete_building(canvas, building_num, building_rects)
...
def check_plane(canvas, plane_pos, building_width, building_heights, building_xpos, building_rects):
    global msg_text_on
    ...
    for building_num in range(0, 1000//SPACING):
        if (is_inside_building(building_num, plane_nose_pos, building_width,
            ...
            game_over(canvas)
            msg_text_on = True
        if CANVAS_HEIGHT <= plane_body_pos[1] < (CANVAS_HEIGHT + 32) and plane_body_pos[0] < 20:
            plane_body_pos[1] = CANVAS_HEIGHT
            plane_landed(canvas)
            msg_text_on = True
...
def game_over(canvas):
    global game_running, won, msg_text, bigfont, msg_text_on
    game_running = False
    won = False
...
def run_game():
    ...
    while prog_running:
        ...
        if game_running:
            ...
            check_plane(canvas, plane_pos, building_width, building_heights, building_xpos, building_rects)
            move_bomb(bomb_pos)
