...
class Building():
    def __init__(self, canvas, building_num, height, width):
        ...
        self.main_rect = canvas.create_rectangle(
            self.x, CANVAS_HEIGHT, self.x + self.width, CANVAS_HEIGHT-self.height, fill="brown")
...
class Bomb():
    def __init__(self, canvas):
        ...
        self.points = [0, 0, 10, 0, 5, 5, 10, 10,
                       10, 20, 5, 22, 0, 20, 0, 10, 5, 5]
        self.draw()
...
class Plane():
    ...
    def draw(self):
        ...
        current_wing1_points = update_position(
            self.wing1_points, self.position)
        current_wing2_points = update_position(
            self.wing2_points, self.position)
        current_tail_points = update_position(self.tail_points, self.position)
        self.body = self.canvas.create_polygon(
            *current_body_points, fill="red")
        self.wing1 = self.canvas.create_polygon(
            *current_wing1_points, fill="grey")
        self.wing2 = self.canvas.create_polygon(
            *current_wing2_points, fill="grey")
        self.tail = self.canvas.create_polygon(
            *current_tail_points, fill="grey")
...
class Display(Frame):
    ...
    def display_score(self):
        self.canvas.itemconfig(self.score_text, text="Level: " + str(
            self.level) + "  Score: " + str(self.score), font=self.scorefont)
